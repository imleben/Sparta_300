# PCOA QZA 파일을 TSV로 변환하고 DQ, MW, TC를 모두 지니는 subject 메타데이터와 병합하는 과정을 반복하는 스크립트.
...
merged_output_folder <- "/users/inseonghwang/onedrive/Sparta_300/05_diversity_20FNS/beta_meta_combined/pcoa2tsv_cs"
...
for (qza_file in qza_files) {
    ...
    # (f) subject별로 그룹화하여, group 컬럼에 DQ, MW, TC 세 가지 모두 포함된 subject만 남기기
    #     (metadata에 subject와 group 컬럼이 포함되어 있어야 함)
    required_groups <- c("DQ", "MW", "TC")
    merged_df <- merged_df %>%
        group_by(subject) %>%
        filter(all(required_groups %in% group)) %>%
        ungroup()

    # (g) 최종 병합 TSV 파일 이름 (.tsv -> _meta_cs.tsv)
    merged_filename <- sub("\\.tsv$", "_meta_cs.tsv", tsv_filename)
